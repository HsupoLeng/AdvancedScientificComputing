...
@testset "Learning Julia 1" begin
    ...
    aanimals =  sort(filter(s -> !in('u', s), split(animals))) # feel free to use multiple lines to solve this; `aanimals` should hold your final answer
    @test aanimals == ["antelope", "cat", "chimp", "dog", "feline", "salamander"]

    # Compute the sum of the ascii codes in `animals`, excluding spaces
    sascii = sum(Int(c) for c in animals if c != ' ')
    @test sascii == 5257

    # What is the most common letter, excluding spaces, in `animals` above?
    ch = [cs[1] for cs in split(animals, "") if !isspace(cs[1])]
    d = Dict{Char, Int64}()    # this variable should be a dictionary
    for c in ch
      d[c] = get(d, c, 0) + 1
    end
    mostcommon = sort(collect(d), by = tup -> last(tup), rev=true)[1][1]   # compute this from `d`
    ...
    chars = Set{Char}(cs[1] for cs in split(animals, ""))
